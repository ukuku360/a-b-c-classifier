...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--label-version",
        default="guideline_v1_1_other_reaudit_round3",
    )
...
def pack_doc_batches(other_df: pd.DataFrame, target_batch_size: int) -> list[pd.DataFrame]:
    ...
    for _, doc_frame in other_df.groupby("doc_id", sort=False):
...
def main() -> None:
    ...
    other_df = winner_predictions.loc[winner_predictions["pred_label"] == OTHER_LABEL].copy()
    other_df = other_df.sort_values(["doc_id", "sent_id", "row_id"]).reset_index(drop=True)

    if other_df.empty:
        raise ValueError("No OTHER rows found in winner predictions")

    batches = pack_doc_batches(other_df, target_batch_size=args.target_batch_size)
    ...
    if len(row_manifest) != len(other_df):
        raise AssertionError("Row manifest length does not match OTHER pool length")
    if row_manifest["row_id"].nunique() != len(other_df):
