# Annotation Process: From `token404_sample_yh` to `master_annotations_round3_reaudit`

## Overview

이 문서는 원본 파일인 `token404_sample_yh.xlsx`에서 출발해 최종 파일인 `data/master_annotations_round3_reaudit.csv`가 만들어지기까지의 전체 과정을 단계별로 설명한다.
...
- 최종 파일: `data/master_annotations_round3_reaudit.csv`
...
원본 `token404_sample_yh.xlsx`는 문장 데이터와 메타데이터, 그리고 기존 힌트 컬럼이 함께 섞여 있는 원시 자료다. 이 상태로는 어노테이션, 모델링, 추적 관리가 어렵기 때문에 먼저 모든 후속 작업의 기준이 되는 canonical sentence table이 필요했다.

### 입력

- 원본 파일: `token404_sample_yh.xlsx`
- 원본 크기: 1,636행, 48개 컬럼
...
- `data/seed_annotations_v1.csv`
- `data/boundary_relabel_queue_v1.csv`
- `data/pilot_summary.json`

### 결과

`data/pilot_summary.json` 기준:
...
현재 `seed_annotations_v1.csv`에는 round1 수동 라벨이 반영되어 있으며, 실제 분포는:
...
- `data/boundary_relabel_queue_v1.csv`
- `outputs/annotation_round1/boundary_disagreements.csv`
- `outputs/annotation_round1/boundary_disagreement_summary.json`
...
- `outputs/pilot_run/model_comparison.csv`
- `outputs/pilot_run/run_summary.json`
- `outputs/pilot_run/winner_predictions.csv`
- `outputs/pilot_run/winner_embeddings.npy`
- `outputs/pilot_run/review_queue_top50.csv`

### 결과

`outputs/pilot_run/run_summary.json` 기준:
...
- `outputs/pilot_run/active_learning_batch_round1.csv`
...
- `data/round2_stage_a_pass1.csv`
- `data/round2_stage_a_blind_pass2.csv`
- `data/round2_stage_b_pass1.csv`
- `outputs/annotation_round2/stage_a_double_check_comparison.csv`
- `outputs/annotation_round2/round2_annotation_summary.json`
...
- `docs/guideline_v1.md`
- `docs/guideline_v1_1.md`
...
- `data/round2_adjudicated_final.csv`
- `outputs/annotation_round2/round2_annotation_summary.json`

### 결과

`round2_adjudicated_final.csv` 기준 최종 분포:
...
현재 `master_annotations.csv`는 300행이며 분포는:
...
- `master_annotations.csv`를 기준으로 baseline 모델을 다시 학습했다.
- 전체 1,636문장에 대한 새로운 winner prediction을 만들었다.
- class별 predicted pool을 확보했다.

### 산출물

- `outputs/pilot_run_round2/model_comparison.csv`
- `outputs/pilot_run_round2/run_summary.json`
- `outputs/pilot_run_round2/winner_predictions.csv`
- `outputs/pilot_run_round2/winner_embeddings.npy`
- `outputs/pilot_run_round2/active_learning_batch_round2.csv`

### 결과

`outputs/pilot_run_round2/run_summary.json` 기준:
...
- `outputs/issue_reaudit_round3/batches/*.csv`
- `outputs/remediation_reaudit_round3/batches/*.csv`
- `outputs/other_reaudit_round3/batches/*.csv`
...
- `outputs/issue_reaudit_round3/issue_reaudit_merged.csv`
- `outputs/issue_reaudit_round3/issue_reaudit_overlay.csv`
- `outputs/issue_reaudit_round3/issue_reaudit_summary.json`
- `outputs/issue_reaudit_round3/issue_reaudit_summary.csv`
...
- 기존 `master_annotations.csv`와 겹치는 73개에 대해서는 overlap comparison도 수행했다.

### 산출물

- `outputs/remediation_reaudit_round3/remediation_reaudit_merged.csv`
- `outputs/remediation_reaudit_round3/remediation_reaudit_overlay.csv`
- `outputs/remediation_reaudit_round3/remediation_reaudit_summary.json`
- `outputs/remediation_reaudit_round3/remediation_reaudit_summary.csv`
- `outputs/remediation_reaudit_round3/master_overlap_comparison.csv`
...
- `outputs/other_reaudit_round3/other_reaudit_merged.csv`
- `outputs/other_reaudit_round3/other_reaudit_overlay.csv`
- `outputs/other_reaudit_round3/other_reaudit_summary.json`
- `outputs/other_reaudit_round3/other_reaudit_summary.csv`
...
- 최종 병합 결과가 `data/sentences.csv`의 1,636개 `row_id`를 정확히 모두 덮는지

### 산출물

- `data/master_annotations_round3_reaudit.csv`
...
- total rows: 1,636
- 기존 `master_annotations.csv`와 overlap: 300
...
최종 round3 파일이 기존 `master_annotations.csv`를 완전히 뒤엎은 것인지, 아니면 일부 보정한 것인지 이해하는 것이 중요하다.
...
`master_annotations_round3_reaudit.csv`는 `token404_sample_yh.xlsx`의 기존 힌트를 그대로 옮긴 파일이 아니라, 문장 표준화, seed 수동 라벨링, 경계 사례 blind 재검토, active learning 기반 추가 라벨링, round2 adjudication, 그리고 round3 전체 1,636문장 전수 재감사를 거쳐 완성된 최종 full master annotation 파일이다.
